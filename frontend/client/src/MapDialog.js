   ...
   return(
    ...
    <Dialog fullScreen TransitionComponent={Transition} open={props.show} onClose={props.closeModal} style={{ backgroundColor: 'black !important' }}>
      ...
      <Toolbar style={{background:'#ffffff',color:"#000000"}}>
          <IconButton edge="start" color="inherit" onClick={()=>{props.closeModal();setDTName("")}} aria-label="close">
          <CloseIcon />
          </IconButton>
          {!DTName && (
          ...
          {DTName && (
              <Typography variant="h6" className={classes.title}>
                  ...
                  {/*<Link to="/"><Button style={{backgroundColor : 'blue',color :'white',float:'right'}}>View Colleges in {DTName}</Button></Link>*/}
